// api/generateFinancialInsights.ts
import type { VercelRequest, VercelResponse } from "@vercel/node";
import { GoogleGenAI, Type } from "@google/genai";
import type { FinancialInput, AIAnalysis } from "../types";

const ai = new GoogleGenAI({
  apiKey: process.env.GEMINI_API_KEY!, // set in .env + Vercel env
});


const preComputeRisks = (
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const concentrationRatio =
      totalAssets > 0 ? input.employerStock / totalAssets : 0;
    const grossIncome = input.baseSalary + input.variableComp;

    const detectedRisks = preComputeRisks(
      input,
      grossIncome,
      concentrationRatio,
      netWorth,
      ...
      publicInsights: [
        {
          ...
          description: "Please discuss the detailed findings with your advisor.",
          status: "good",
        },
      ],
